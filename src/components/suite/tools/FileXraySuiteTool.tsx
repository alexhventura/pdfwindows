    ...
    <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-1 py-2 border-b border-slate-100 last:border-0">
      <div className="min-w-0 flex-1">
        ...
        <p className="text-sm font-semibold text-slate-800 break-words">
          {value}
          {estimated ? (
            <span className="ml-2 text-[10px] font-bold text-amber-600 uppercase">{t.estimate}</span>
          ) : null}
        </p>
        {origin ? (
      ...
      <button
        type="button"
        className="shrink-0 inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider text-win-blue hover:bg-blue-50 px-2 py-1 rounded-lg"
        onClick={async () => {
          try {
...
export default function FileXraySuiteTool({
  ...
  if (busy) {
    return <ToolBusyState label={stage || t.analyzing} />;
  }
      ...
      <div className="space-y-5">
        <div>
          <div className="flex items-center gap-2 mb-2">
            <ScanSearch className="text-win-blue" size={22} aria-hidden />
            <h2 className="text-lg font-bold text-slate-900">{t.title}</h2>
          </div>
          <p className="text-sm text-slate-500 leading-relaxed">{t.hero}</p>
        </div>
        ...
        <p className="flex items-start gap-2 text-[11px] text-slate-500 font-medium">
          <ShieldCheck size={14} className="shrink-0 mt-0.5 text-emerald-600" aria-hidden />
          {t.privacy}
        </p>
      </div>
    );
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-[10px] font-bold uppercase tracking-wider text-emerald-600 mb-1">{t.done}</p>
          <h2 className="text-lg font-bold text-slate-900">{t.title}</h2>
          <p className="text-xs text-slate-500 mt-1">{id.name}</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <button
            type="button"
            onClick={reset}
            className="inline-flex items-center gap-1.5 text-xs font-bold text-slate-600 hover:bg-slate-100 px-3 py-2 rounded-xl"
          >
      ...
      <section aria-label={t.overview} className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
        {summaryCards.map((card) => (
          <div key={card.label + card.value} className="rounded-xl border border-slate-150 bg-slate-50/80 px-3 py-2.5">
            <p className="text-[9px] font-bold uppercase tracking-wider text-slate-400">{card.label}</p>
            <p className="text-xs font-bold text-slate-800 mt-0.5 break-all">{card.value}</p>
          </div>
        ))}
      </section>

      {id.inconsistency ? (
        <div className="flex items-start gap-2 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2.5 text-[11px] font-semibold text-amber-900">
      ...
      {result.hidden.length > 0 ? (
        <section className="rounded-2xl border border-slate-200 bg-white p-4">
          <h3 className="text-sm font-bold text-slate-900 mb-3">{t.additional}</h3>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-4 space-y-1">
        <h3 className="text-sm font-bold text-slate-900 mb-2">{t.identification}</h3>
      ...
      {showDetails ? (
        <>
          <section className="rounded-2xl border border-slate-200 bg-white p-4 space-y-1">
            <h3 className="text-sm font-bold text-slate-900 mb-2">{t.authorship}</h3>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-4 space-y-1">
            <h3 className="text-sm font-bold text-slate-900 mb-2">{t.dates}</h3>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-4 space-y-1">
            <h3 className="text-sm font-bold text-slate-900 mb-2">{t.statistics}</h3>
          ...
          {result.fonts.length > 0 ? (
            <section className="rounded-2xl border border-slate-200 bg-white p-4">
              <h3 className="text-sm font-bold text-slate-900 mb-2">{t.fonts}</h3>
          ...
          {result.structure.length > 0 ? (
            <section className="rounded-2xl border border-slate-200 bg-white p-4 space-y-1">
              <h3 className="text-sm font-bold text-slate-900 mb-2">{t.structure}</h3>
          ...
          {result.metadata.length > 0 ? (
            <section className="rounded-2xl border border-slate-200 bg-white p-4 space-y-1">
              <h3 className="text-sm font-bold text-slate-900 mb-2">{t.metadata}</h3>
          ...
          {(result.security.encrypted != null || result.security.notes.length > 0) && (
            <section className="rounded-2xl border border-slate-200 bg-white p-4 space-y-2">
              <h3 className="text-sm font-bold text-slate-900 mb-2">{t.security}</h3>
          ...
          {result.image?.gps ? (
            <section className="rounded-2xl border border-slate-200 bg-white p-4 space-y-1">
              <h3 className="text-sm font-bold text-slate-900 mb-2">GPS</h3>
              <FieldRow
                label="Coordinates"
          ...
          {result.content.sheets && result.content.sheets.length > 0 ? (
            <section className="rounded-2xl border border-slate-200 bg-white p-4">
              <h3 className="text-sm font-bold text-slate-900 mb-2">Sheets</h3>
              ...
              {result.content.formulaBreakdown?.length ? (
                <div className="mt-3 flex flex-wrap gap-2">
                  {result.content.formulaBreakdown.map((f) => (
                    <span key={f.fn} className="text-[10px] font-bold bg-slate-100 px-2 py-1 rounded-lg text-slate-700">
                      {f.fn}: {f.count}
                    </span>
                  ))}
                </div>
      ...
      {showAdvanced ? (
        <>
          {result.embedded.length > 0 ? (
            <section className="rounded-2xl border border-slate-200 bg-white p-4">
              <h3 className="text-sm font-bold text-slate-900 mb-2">{t.embedded}</h3>
          ...
          {result.advanced.length > 0 ? (
            <section className="rounded-2xl border border-slate-200 bg-white p-4 space-y-1">
              <h3 className="text-sm font-bold text-slate-900 mb-2">{t.advancedData}</h3>
      ...
      <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-slate-100">
        <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mr-1">{t.export}</span>
        <button
          type="button"
          className="inline-flex items-center gap-1.5 text-xs font-bold bg-win-blue text-white px-3 py-2 rounded-xl"
          onClick={async () => {
        ...
        <button
          type="button"
          className="inline-flex items-center gap-1.5 text-xs font-bold bg-slate-800 text-white px-3 py-2 rounded-xl"
        ...
        <button
          type="button"
          className="inline-flex items-center gap-1.5 text-xs font-bold border border-slate-200 text-slate-700 px-3 py-2 rounded-xl"
